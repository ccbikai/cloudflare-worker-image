...
use image::{
    ...
    ColorType, GenericImageView, ImageBuffer, ImageFormat, Rgba,
};
use photon_rs::{
...
pub async fn process_image(
    ...
    if let Some(action) = &query.action {
        let actions = action.split('|').filter(|s| !s.is_empty());
            ...
            match action_name {
                ...
                "rotate" => {
                    if !params.is_empty() {
                        ...
                        img = transform::rotate(&img, angle as f32);
                    }
                }
    ...
    match output_format {
        ...
        ImageFormat::Png => {
            ...
            let compression = match quality {
                Some(q) if q <= 9 => image::codecs::png::CompressionType::Best, // Assuming higher number is better
            ...
            dyn_image.write_with_encoder(encoder).map_err(|e| {
                eprintln!(
                    "[ERROR] url: {} Failed to encode PNG: {}",
                    query.url, e
                );
                error::ErrorInternalServerError(format!(
                    "Failed to encode PNG for url {}: {}",
                    query.url, e
                ))
            })?;
        }
        ImageFormat::WebP => {
            ...
            dyn_image.write_with_encoder(encoder).map_err(|e| {
                eprintln!(
                    "[ERROR] url: {} Failed to encode WebP: {}",
                    query.url, e
                );
                error::ErrorInternalServerError(format!(
                    "Failed to encode WebP for url {}: {}",
                    query.url, e
                ))
            })?;
        }
        ImageFormat::Avif => {
            ...
            dyn_image.write_with_encoder(encoder).map_err(|e| {
                eprintln!(
                    "[ERROR] url: {} Failed to encode AVIF: {}",
                    query.url, e
                );
                error::ErrorInternalServerError(format!(
                    "Failed to encode AVIF for url {}: {}",
                    query.url, e
                ))
            })?;
        }
