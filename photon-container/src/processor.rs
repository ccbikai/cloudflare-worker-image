...
pub async fn process_image(
    ...
    let dyn_image = image::load_from_memory(&image_bytes).map_err(AppError::ImageDecodeError)?;
    ...
    let image_buffer =
        ImageBuffer::<Rgba<u8>, _>::from_raw(img.get_width(), img.get_height(), img.get_raw_pixels())
...
async fn fetch_image(url: &str) -> Result<Bytes, AppError> {
    let res = reqwest::get(url).await.map_err(AppError::FetchError)?;

    if !res.status().is_success() {
        return Err(AppError::FetchStatusError {
            url: url.to_string(),
            status: StatusCode::from_u16(res.status().as_u16()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
        });
    }

    res.bytes().await.map_err(AppError::FetchError)
}
